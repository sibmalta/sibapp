-- Dispute conversation threads for marketplace resolution center.
...
CREATE INDEX IF NOT EXISTS idx_dispute_conversations_participants
  ON public.dispute_conversations(buyer_id, seller_id);
...
SET
  ...
  message_type = CASE WHEN dm.sender_role = 'system' THEN 'system' ELSE coalesce(dm.message_type, 'message') END
...
DECLARE
  ...
  v_notify_title TEXT := 'New dispute update';
  v_notify_body TEXT;
