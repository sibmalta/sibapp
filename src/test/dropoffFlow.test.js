...
describe('seller drop-off flow', () => {
  ...
  it('keeps the seller drop-off detail screen focused on QR-only store scan instructions', () => {
    ...
    expect(detail).toContain('Write the order number and buyer name clearly on the outside of the parcel.')
    expect(detail).toContain('Write on parcel')
    expect(detail).toContain('Order number')
    expect(detail).toContain('Buyer')
    expect(detail).toContain('Write both clearly on the outside of the package before handing it to MYConvenience.')
