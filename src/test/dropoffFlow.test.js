...
describe('seller drop-off flow', () => {
  ...
  it('includes the QR drop-off confirmation migration and admin scan route', () => {
    ...
    const storeMigration = readFileSync(resolve(root, 'supabase/migrations/20260505143000_public_dropoff_store_location.sql'), 'utf8')
    const storeTableMigration = readFileSync(resolve(root, 'supabase/migrations/20260505160000_myconvenience_stores.sql'), 'utf8')
    const storeDiagnosticsMigration = readFileSync(resolve(root, 'supabase/migrations/20260512103000_myconvenience_store_diagnostics.sql'), 'utf8')
    const app = readFileSync(resolve(root, 'src/App.jsx'), 'utf8')
    const scanPage = readFileSync(resolve(root, 'src/pages/AdminScanDropoffPage.jsx'), 'utf8')
    const publicScan = readFileSync(resolve(root, 'src/lib/publicDropoffScan.js'), 'utf8')
    ...
    expect(storeTableMigration).not.toContain('longitude')
    expect(storeMigration).toContain('dropoff_location_name')
    expect(storeMigration).toContain('dropoff_store_id')
    expect(storeTableMigration).toContain('logistics_delivery_sheet')
    ...
    expect(storeDiagnosticsMigration).toContain('normalize_myconvenience_store_pin')
    expect(storeDiagnosticsMigration).toContain('admin_myconvenience_store_diagnostics')
    expect(storeDiagnosticsMigration).toContain('store_pin_hash IS NOT NULL')
    expect(storeDiagnosticsMigration).toContain('has_pin_hash BOOLEAN')
    expect(storeDiagnosticsMigration).toContain('GRANT EXECUTE ON FUNCTION public.admin_myconvenience_store_diagnostics() TO authenticated')
    ...
    expect(importScript).toContain('store_pin: String(row.store_pin || \'\').trim()')
    expect(importScript).toContain('upsert_myconvenience_store')
    expect(importScript).toContain('Every active MYConvenience store must have a store_pin')
    ...
    expect(scanPage).toContain('identifyPublicDropoffStoreByPin')
    expect(scanPage).toContain('listMyConvenienceStoreDiagnostics')
    expect(scanPage).toContain('MYConvenience store PIN')
    expect(scanPage).toContain('Ask store staff to enter their store PIN.')
    expect(scanPage).toContain('Verify store PIN')
    expect(scanPage).not.toContain('fetchPublicDropoffStores')
    expect(publicScan).toContain('identify_public_dropoff_store_by_pin')
    expect(publicScan).toContain('admin_myconvenience_store_diagnostics')
    expect(publicScan).toContain('normalizeStorePin')
    expect(publicScan).toContain('p_store_pin')
    expect(publicScan).not.toContain(".from('myconvenience_stores')")
    expect(publicScan).not.toContain('store_pin_hash')
    expect(scanPage).not.toContain('PUBLIC_DROPOFF_STORES')
    expect(scanPage).not.toContain('Select store')
    expect(scanPage).not.toContain('<select')
    expect(scanPage).toContain('!canConfirm || confirming')
    ...
    expect(scanPage).toContain('currentUser?.isAdmin')
  ...
  it('keeps the seller drop-off detail screen focused on QR-only store scan instructions', () => {
    ...
    expect(detail).toContain('Delivery partner')
    expect(detail).toContain('Delivery method')
    expect(detail).toContain('Delivery fee')
    expect(detail).toContain('Current delivery status')
    expect(detail).not.toContain('Fulfilment provider')
    expect(detail).not.toContain('Fulfilment method')
    expect(detail).not.toContain('Fulfilment price')
    expect(detail).not.toContain('Current fulfilment status')
