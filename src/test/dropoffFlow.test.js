...
describe('seller drop-off flow', () => {
  ...
  it('routes the seller prompt CTA to the multi-order drop-off QR page', () => {
    ...
    expect(layout).toContain("pathname === '/dropoff'")
    expect(layout).toContain("pathname.startsWith('/dropoff/')")
    expect(layout).toContain('showSellerDropoffPrompt && <SellerDropoffPrompt />')
  ...
  it('keeps the seller drop-off detail screen focused on QR-only store scan instructions', () => {
    ...
    expect(detail).toContain('IMPORTANT: Write clearly on parcel')
    ...
    expect(detail).not.toContain('Write on parcel')
