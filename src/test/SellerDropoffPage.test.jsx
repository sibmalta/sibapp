...
describe('SellerDropoffPage', () => {
  ...
  it('clicking Show QR reveals the QR for that parcel only', () => {
    ...
    expect(screen.getByText('REQUIRED: Write on parcel')).toBeInTheDocument()
    expect(screen.getByText('Required for delivery — parcels without this may be delayed')).toBeInTheDocument()
    expect(screen.getByText('Parcels without these details may be delayed or returned.')).toBeInTheDocument()
