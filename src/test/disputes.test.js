...
describe('dispute handling helpers', () => {
  ...
  it('defines dispute conversation threads, internal notes, public replies, and notifications', () => {
    ...
    const appRoutes = readFileSync(resolve(root, 'src/App.jsx'), 'utf8')
    const appContext = readFileSync(resolve(root, 'src/context/AppContext.jsx'), 'utf8')
    ...
    const authPage = readFileSync(resolve(root, 'src/pages/AuthPage.jsx'), 'utf8')
    const authCallbackPage = readFileSync(resolve(root, 'src/pages/AuthCallbackPage.jsx'), 'utf8')
    ...
    expect(appRoutes).toContain('messages/dispute/:disputeId')
    ...
    expect(notificationsPage).toContain('dispute_opened:')
    expect(notificationsPage).toContain('dispute_opened_buyer:')
    expect(notificationsPage).toContain('dispute_resolved:')
    expect(notificationsPage).toContain("String(notif.type || '').startsWith('dispute_')")
    expect(chatPage).toContain('isDisputeThread')
    expect(chatPage).toContain('Loading dispute conversation')
    expect(chatPage).toContain('Dispute conversation not found')
    expect(chatPage).toContain("supabase.rpc('ensure_dispute_conversation'")
    expect(chatPage).toContain(".from('disputes')")
    expect(chatPage).toContain(".from('dispute_messages')")
    expect(chatPage).toContain('buildDisputeThreadConversation')
    expect(chatPage).toContain('sib_auth_redirect')
    expect(chatPage).toContain('addDisputeMessage')
    expect(authPage).toContain('sib_auth_redirect')
    expect(authCallbackPage).toContain('sib_auth_redirect')
    expect(sendEmail).toContain('const disputeThreadUrl')
    expect(sendEmail).toContain("const PRODUCTION_APP_URL = 'https://www.sibmalta.com'")
    expect(sendEmail).toContain('const disputeIdFromPayload')
    ...
    expect(appContext).toContain('const disputeTarget = `/messages/dispute/${newDispute.id}`')
    expect(appContext).toContain('const disputeTarget = `/messages/dispute/${dispute.id}`')
    expect(buyerProtection).toContain('target_path: `/messages/dispute/${dispute.id}`')
    expect(buyerProtection).toContain('action_target: `/messages/dispute/${dispute.id}`')
  ...
  it('hardens dispute conversation access while allowing buyer seller and admin thread loading', () => {
    const root = resolve(__dirname, '..', '..')
    const migration = readFileSync(resolve(root, 'supabase/migrations/20260509170000_harden_dispute_conversation_access.sql'), 'utf8')

    expect(migration).toContain('CREATE OR REPLACE FUNCTION public.ensure_dispute_conversation')
    expect(migration).toContain('v_actor UUID := auth.uid()')
    expect(migration).toContain('v_is_admin BOOLEAN := public.is_admin()')
    expect(migration).toContain('v_actor = v_dispute.buyer_id')
    expect(migration).toContain('v_actor = v_dispute.seller_id')
    expect(migration).toContain("RAISE EXCEPTION 'not_allowed'")
    expect(migration).toContain('GRANT EXECUTE ON FUNCTION public.ensure_dispute_conversation(UUID) TO authenticated')
  })
})
