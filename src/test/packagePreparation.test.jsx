...
describe('PackagePreparationModal', () => {
  ...
  it('shows the package preparation checklist', () => {
    render(<PackagePreparationModal />)

    expect(screen.getByText('Prepare your package for pickup')).toBeInTheDocument()
    ...
    expect(screen.getByText('Attach the Sib delivery label when provided')).toBeInTheDocument()
    expect(screen.getByText('Keep the package ready at the pickup address')).toBeInTheDocument()
    expect(screen.getByText('Hand it only to an approved drop-off partner or courier')).toBeInTheDocument()
  })
  ...
  it('marks the offer package as prepared from the CTA', async () => {
    ...
    expect(mockApp.showToast).toHaveBeenCalledWith('Package marked ready for pickup.')
  })
})
