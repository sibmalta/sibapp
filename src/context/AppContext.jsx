...
import {
  sendOfferReceivedEmail, sendOfferAcceptedEmail,
  sendOrderConfirmedEmail, sendPaymentConfirmedEmail,
  sendItemShippedEmail, sendItemDeliveredEmail,
  sendRefundConfirmedEmail, sendDisputeOpenedEmail,
  sendItemSoldEmail, sendShippingReminderEmail, sendPayoutReleasedEmail,
  sendSuspiciousActivityEmail, sendModerationNoticeEmail,
} from '../lib/email'
...
function buildAppUser(authUser) {
  ...
  return {
    ...
    phone: meta.phone || '',
    bio: meta.bio || '',
    avatar: meta.avatar || meta.avatar_url || `https://i.pravatar.cc/150?u=${authUser.email}`,
...
export function AppProvider({ children }) {
      ...
      for (const s of shipments) {
        ...
        if (elapsed >= SHIPPING_REMINDER_MS) {
          ...
          addNotification({
            ...
            message: `Order ${s.orderRef || s.orderId} must be shipped within 24 hours to avoid cancellation.`,
          })
          const seller = users.find(u => u.id === s.sellerId)
          if (seller?.email) {
            sendShippingReminderEmail(seller.email, seller.name, s.orderRef || s.orderId)
          }
        }
      }
    }
  ...
  const placeOrder = useCallback(async (listingId, deliveryMethod, address, overridePrice) => {
    ...
    // Notify seller about shipping deadline
    addNotification({
      userId: listing.sellerId,
      orderId: savedOrder.id,
      type: 'ship_reminder',
      title: 'New sale — ship within 3 days',
      message: `You have a new order (${orderRef}). Please ship via MaltaPost within 3 business days.`,
    })

    // Email: order confirmation to buyer
    if (currentUser?.email) {
      sendOrderConfirmedEmail(currentUser.email, currentUser.username, orderRef, listing.title, totalPrice.toFixed(2), 'Sib Delivery')
    }
  ...
  const confirmDelivery = useCallback(async (orderId) => {
    ...
    if (order) {
      ...
      const listing = listings.find(l => l.id === order.listingId)
      if (seller?.email) {
        sendPayoutReleasedEmail(seller.email, seller.name, listing?.title || 'item', order.sellerPayout?.toFixed(2) || '0.00')
      }
    }
  ...
  const openDispute = useCallback(async (orderId, reason, opts = {}) => {
    ...
    const buyer = users.find(u => u.id === order.buyerId)
    if (buyer?.email) {
      sendDisputeOpenedEmail(buyer.email, buyer.name, order.orderRef || orderId, description)
    }
  ...
  const flagOrderOverdue = useCallback(async (orderId) => {
    ...
    if (order) {
      addNotification({
        ...
        message: 'Your shipment is overdue. Please ship the item immediately to avoid order cancellation.',
      })
    }
  ...
  const placeBundleOrder = useCallback(async (address, overrideSubtotal) => {
    ...
    addNotification({
      ...
      message: `@${currentUser.username} purchased ${items.length} items for €${fees.total.toFixed(2)}. Ship via MaltaPost within 3 business days.`,
    })

    // Email buyer order confirmation
    sendOrderConfirmedEmail(currentUser.email, currentUser.username, orderRef, `${items.length}-item bundle`, fees.total.toFixed(2), 'Sib Delivery')

    // Clear bundle after order
    setBundle(null)
    return savedOrder
  }, [currentUser, bundle, listings, calculateBundleFees, addNotification, dbCreateOrder, dbCreateShipment, showToast])
  ...
  const placeBundleOfferOrder = useCallback(async (offerId, address) => {
    const offer = bundleOffers.find(o => o.id === offerId)
    ...
    addNotification({
      ...
      type: 'bundle_sold',
      title: `${items.length}-item bundle sold — ship within 3 days`,
      message: `@${users.find(u => u.id === offer.buyerId)?.username || 'buyer'} purchased ${items.length} items for €${fees.total.toFixed(2)}. Ship via MaltaPost within 3 business days.`,
    })

    // Email buyer order confirmation
    const buyer = users.find(u => u.id === offer.buyerId)
    if (buyer) {
      sendOrderConfirmedEmail(buyer.email, buyer.username, orderRef, `${items.length}-item bundle`, fees.total.toFixed(2), 'Sib Delivery')
    }
  ...
  const releasePayout = useCallback(async (orderId) => {
    ...
    const seller = users.find(u => u.id === order.sellerId)
    const listing = listings.find(l => l.id === order.listingId)
    if (seller?.email) sendPayoutReleasedEmail(seller.email, seller.name, listing?.title || 'item', order.sellerPayout?.toFixed(2) || '0.00')
  ...
  const refundOrder = useCallback(async (orderId) => {
    ...
    if (order) {
      const buyer = users.find(u => u.id === order.buyerId)
      const listing = listings.find(l => l.id === order.listingId)
      if (buyer?.email) sendRefundConfirmedEmail(buyer.email, buyer.name, listing?.title || 'item', order.totalPrice?.toFixed(2) || '0.00')
    }
  }, [orders, users, listings, authSession, refreshOrders, showToast])
  ...
  }, [disputes, dbPatchDispute, refundOrder, releasePayout, showToast])
  ...
  }, [disputes, addNotification, dbPatchDispute, showToast])
  ...
  return (
    <AppContext.Provider value={{
      currentUser, users, listings, orders, conversations, reviews, disputes, likedListings, payoutProfiles, notifications, offers, bundle, bundleOffers, shipments, toast,
      ...
      createListing, deleteListing, boostListing, unboostListing, flagListing, approveListing, hideListing, toggleLike,
      ...
      suspendUser, banUser, restoreUser, holdPayout, releasePayout, refundOrder, resolveDispute, cancelOrder, addDisputeMessage,
