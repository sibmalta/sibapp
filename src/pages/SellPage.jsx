...
import { Camera, Plus, X, Info, ImagePlus, Check } from 'lucide-react'
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!listing || !listing.id) {
        showToast('Failed to create listing — no data returned. Check console for details.', 'error')
        setUploading(false)
        return
      }
              ...
              {COLOURS.map(c => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (selected) {
                        set('colors', form.colors.filter(v => v !== c.value))
                      } else if (!atMax) {
                        set('colors', [...form.colors, c.value])
                      }
                    }}
                    disabled={atMax}
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                        selected ? 'ring-2 ring-sib-primary ring-offset-2' : ''
                      } ${c.border ? 'border border-sib-stone' : ''}`}
                      style={{ background: c.isGradient ? c.hex : c.hex }}
                    >
                      {selected && (
                        <Check size={14} className={c.value === 'white' || c.value === 'yellow' || c.value === 'beige' ? 'text-sib-text' : 'text-white'} />
                      )}
                    </div>
        ...
        <>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={uploading}
              className="flex-1 bg-sib-secondary text-white font-bold py-4 rounded-2xl text-sm disabled:opacity-50"
            >
