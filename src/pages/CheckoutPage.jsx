...
export default function CheckoutPage() {
  ...
  const handlePaymentSuccess = async (stripePaymentIntentId) => {
    ...
    const order = await placeOrder(id, deliveryMethodId, getFullAddress(), undefined, deliveryInfo)
    if (order) {
      try {
    ...
    <div>
      ...
      <div className="p-4 rounded-2xl bg-sib-warm mb-5 lg:hidden">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="text-xs text-sib-muted mt-0.5 pl-[22px]">est. {deliveryMethod?.estimatedDays}</div>
