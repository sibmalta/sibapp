...
import {
  ...
  Flag, Trash2, EyeOff, Send, AlertOctagon, Mail,
} from 'lucide-react'
...
export default function AdminPage() {
  const {
    ...
    deleteListing, flagListing, hideListing,
                        ...
                        {!u.isAdmin && (
                          <div className="flex gap-1.5">
                            {!u.banned ? (
                              ...
                              <button onClick={() => { restoreUser(u.id); showToast(`${u.name} unbanned`) }}
          ...
          <div>
            ...
            <div className="space-y-2">
              {filteredListings.map(l => {
                const owner = getUserById(l.userId)
                return (
                  <div key={l.id} className="p-3 rounded-2xl border border-sib-ash">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-sib-muted">€{l.price?.toFixed(2)} · {owner?.name || '?'}</p>
