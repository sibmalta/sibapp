...
export default function BrowsePage() {
  ...
  // Count active listings per canonical brand (for ShopByBrand badges)
  const brandCounts = useMemo(() => {
    const counts = {}
    for (const l of activeListings) {
      if (!l.brand) continue
      const canonical = normalizeBrand(l.brand)
      if (canonical) counts[canonical] = (counts[canonical] || 0) + 1
    }
    return counts
  }, [activeListings])

  // Sync URL ?brand= param on mount (deep-link support)
  useEffect(() => {
    const urlBrand = searchParams.get('brand')
    if (urlBrand) {
      const canonical = normalizeBrand(urlBrand)
      if (canonical) setBrands([canonical])
    }
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  // Handler for ShopByBrand tile clicks — toggles a single brand filter
  const handleBrandSelect = (brandName) => {
    setBrands(prev => {
      const exists = prev.some(b => b.toLowerCase() === brandName.toLowerCase())
      return exists ? prev.filter(b => b.toLowerCase() !== brandName.toLowerCase()) : [brandName]
    })
  }
