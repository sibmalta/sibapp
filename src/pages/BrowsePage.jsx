...
import { SlidersHorizontal, X, Search, Sparkles, Tag, TrendingUp, Gem, Clock, DollarSign, ShieldCheck } from 'lucide-react'
...
export default function BrowsePage() {
  ...
  const results = useMemo(() => {
    ...
    if (colors.length) {
      filtered = filtered.filter(l => {
        if (l.color && colors.includes(l.color)) return true
  ...
  }, [activeListings, query, category, conditions, sizes, colors, brands, maxPrice, sort, quickFilter])
  ...
  const activeChips = useMemo(() => {
    ...
    return chips
  }, [conditions, sizes, colors, brands, maxPrice])
          ...
          {(category === 'vintage' || quickFilter === 'vintage') && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 mb-2.5 lg:mb-3 bg-amber-50 border border-amber-200/60 rounded-xl">
              ...
              <p className="text-[12px] lg:text-[13px] leading-snug text-amber-800">
                Look for the <span className="font-bold">Vintage Verified</span> badge to shop curated vintage with more confidence.
              </p>
            </div>
          )}
