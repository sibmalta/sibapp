...
export function sendDisputeOpenedEmail(recipientEmail, recipientName, orderRef, reason, role = 'buyer') {
  sendEmail('dispute_opened', recipientEmail, { recipientName, orderRef, reason, role })
}
...
export function sendOfferDeclinedEmail(buyerEmail, itemTitle, declinedPrice, sellerName) {
  sendEmail('offer_declined', buyerEmail, { itemTitle, declinedPrice, sellerName })
}

export function sendOfferCounteredEmail(buyerEmail, itemTitle, originalPrice, counterPrice, sellerName) {
  sendEmail('offer_countered', buyerEmail, { itemTitle, originalPrice, counterPrice, sellerName })
}

// ── ORDER CANCELLED EMAIL ─────────────────────────────────

export function sendOrderCancelledEmail(buyerEmail, buyerName, orderRef, itemTitle, refundAmount) {
  sendEmail('order_cancelled', buyerEmail, { buyerName, orderRef, itemTitle, refundAmount })
}

export function sendOrderCancelledSellerEmail(sellerEmail, sellerName, orderRef, itemTitle) {
  sendEmail('order_cancelled_seller', sellerEmail, { sellerName, orderRef, itemTitle })
}

// ── DISPUTE RESOLUTION EMAILS ─────────────────────────────

export function sendDisputeResolvedEmail(userEmail, userName, orderRef, resolution) {
  sendEmail('dispute_resolved', userEmail, { userName, orderRef, resolution })
}

export function sendDisputeMessageEmail(userEmail, userName, orderRef, messagePreview) {
  sendEmail('dispute_message', userEmail, { userName, orderRef, messagePreview })
}

// ── BUNDLE OFFER EMAILS ───────────────────────────────────

export function sendBundleOfferReceivedEmail(sellerEmail, itemCount, offerPrice, originalTotal, buyerName) {
  sendEmail('bundle_offer_received', sellerEmail, { itemCount, offerPrice, originalTotal, buyerName })
}

export function sendBundleOfferAcceptedEmail(buyerEmail, itemCount, acceptedPrice, sellerName) {
  sendEmail('bundle_offer_accepted', buyerEmail, { itemCount, acceptedPrice, sellerName })
}

export function sendBundleOfferDeclinedEmail(buyerEmail, itemCount, declinedPrice, sellerName) {
  sendEmail('bundle_offer_declined', buyerEmail, { itemCount, declinedPrice, sellerName })
}

export function sendBundleOfferCounteredEmail(buyerEmail, itemCount, originalPrice, counterPrice, sellerName) {
  sendEmail('bundle_offer_countered', buyerEmail, { itemCount, originalPrice, counterPrice, sellerName })
}

// ── TRUST & SAFETY EMAILS ────────────────────────────────

export function sendSuspiciousActivityEmail(userEmail, userName, message) {
  sendEmail('suspicious_activity', userEmail, { userName, message })
}
